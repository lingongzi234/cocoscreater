...
// 推箱子核心：1.地图用数字表示类型 2.palace[][]存逻辑、landArrays[][]存显示 3.点到空地寻路、点到箱子判断能否推 4.寻路用DFS 5.胜利=finishBoxCount==allBox
...
cc.Class({
    ...
    onLoad : function(){
        ...
        // 加载 levelConfig.json，拿到所有关卡的 content、allRow、allCol、heroRow/Col、allBox
        this.initData();
    ...
    //onLoad：curLayer=0、clearGameData、initData 加载 levelConfig、显示主界面并禁用开始按钮直到配置加载完
    ...
    //fitNode：按设计 720*1280 比例适配背景高，使宽铺满时高度按比例缩放
    ...
    initData : function(){
        ...
        this.tabLevel = [];  // 关卡项组件引用，用于刷新星星/解锁状态
    },
    // content 一维数组：下标 = 行*allCol+列，与 levelConfig.json 里每关的 content 对应

    // 根据关卡总数实例化关卡项（levelItemPrefab），放入 levelScroll，并设置 levelContent 高度
    createLavelItem (){
        // 进入关卡level
        let callfunc = level => {            
    ...
    // 【学习】关卡项点击时调用 selectLevelCallBack(level)，level 来自 levelItem 的 _level_
    ...
    // localStorage：finishLevel=已过关最大号，levelStar1/2/3...=星星数，levelBest1/2/3...=最优步数
    ...
    refreshCallBack : function(event, customEventData){
        this.playSound(sound.BUTTON);
        //停止人运动动作
        // 【学习】重置 = 停止所有动作 + clearGameData + createLevelLayer(curLevel)，即本关从头再来
    ...
    createLevelLayer(level){
        // 每关重建：先清空 gameControlLayer，再从 levelConfig[level] 取 content/allRow/allCol/heroRow/heroCol/allBox，建 landArrays+palace
    ...
    //landArrays[row][col]=显示节点，palace[row][col]=逻辑类型；坐标 x=sPosX+boxW*j，y=sPosY-boxH*i+offset（行增向下）
    ...
    //只有 LAND/BODY/BOX/ENDBOX 会建可点击按钮；节点 attr 存 _row_ _col_，点击时用 target._row_/_col_ 取格子
    ...
    //点击分两类：点到空地/目标点→getPath 寻路再 runHero 走过去；点到箱子→必须相邻且下一格可放（非墙非箱）才推，并更新 palace+setLandFrame+checkGameOver
    ...
    // DFS 回溯寻路：只走上下左右，走过格子暂时标成 WALL 防止重复，回溯时恢复；找到终点且步数更短则更新 bestMap/minPath
    ...
    //runHero 用 cc.sequence 串起 delay+callFunc：每步更新 setLandFrame(当前格 HERO、上一格还原)，最后更新 heroRow/heroCol 并 setLandEnable(true)
    ...
    //setLandFrame 只改显示（Sprite 的 spriteFrame），逻辑以 palace 为准；图集里 p0~p6 对应 boxType 的 NONE~HERO
    ...
    //过关条件：finishBoxCount == allBox；过关后写 localStorage（finishLevel、levelStar、levelBest），再 clearGameData
    ...
    //clearGameData 只清 finishBoxCount、curStepNum；过关或重置时调用，不清 landArrays/palace（重建关卡时会覆盖）
